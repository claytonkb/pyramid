...
void *_newlf(bvm_cache *this_bvm, mword size){ // WAIVER_REQUIRED _newlf#
    ...
    void *ptr = (void*)mem_alloc(this_bvm, UNITS_MTO8(size));
...
void *_sys_cp(bvm_cache *this_bvm, mword *bs){ // WAIVER_REQUIRED _sys_cp#
    ...
    mword byte_size = UNITS_MTO8(size(bs));
...
//    }
//    else{
//        memset((char*)ptr,init,UNITS_MTO8(size));
//    }
...
inline void *_newin(bvm_cache *this_bvm, mword size){ // _newin#
    ...
    void *ptr = (void*)mem_alloc(this_bvm, -1*UNITS_MTO8(size)); // FIXME: Bare constant should be macro
...
#define MU_D_OPERATIONS \
    result0 = _val(this_bvm, UNITS_MTO8( _mu(this_bvm, oi0.data) ) );
