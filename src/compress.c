...
static unsigned int   bwt_block_size;
static unsigned char *bwt_block_ptr;
...
//
//
mword *compress_bwt_encode_block(pyr_cache *this_pyr, unsigned char *start, int block_size){ // compress_bwt_encode_block#

//    assert(block_size < BWT_MAX_BLOCK_SIZE);
//    assert(block_size > BWT_MIN_BLOCK_SIZE);

    bwt_block_size = block_size;
    bwt_block_ptr  = start;

    val bwt_indices = mem_new_valz(this_pyr, block_size);

    unsigned char *result = (unsigned char*)_newstr(this_pyr, block_size+BWT_START_INDEX_WIDTH, ' '); // +BWT_START_INDEX_WIDTH to store index of start character
    ...
    for(i=0;i<block_size;i++){
        if(bwt_indices[i] == 0) start_index=i;
    }
...
//
mword *compress_bwt_decode_block(pyr_cache *this_pyr, unsigned char *start, int block_size){ // compress_bwt_decode_block#

    block_size-=BWT_START_INDEX_WIDTH;

    val letter_counts = mem_new_valz(this_pyr, block_size);
    val lex_counts    = mem_new_valz(this_pyr, BWT_COUNT_TABLE_SIZE);
    unsigned char *result = (unsigned char*)_newstr(this_pyr, block_size, '\0');
...
// full-block comparison with "wrap-around"
//
int compress_bwt_compare_encode(const void *a, const void *b){ // compress_bwt_compare_encode#
    ...
    unsigned int ind_a = *bwt_a;
    unsigned int ind_b = *bwt_b;
...
void compress_mtf_encode(pyr_cache *this_pyr, val8 in){ // compress_mtf_encode#
    ...
    val byte_table = mem_new_valz(this_pyr, MTF_BYTE_TABLE_SIZE);
...
//_d(range_lo);
            ...
            while((range_hi & 0xff000000) == (range_lo & 0xff000000)){ // FIXME: 32-bit specific FIXME //
                count++;
