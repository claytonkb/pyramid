...
_trace;
    ...
    mword *load_bbl = (void*)mem_alloc(this_bvm, UNITS_MTO8(BBL_SIZE)); // BBL_SIZE is sfield/4
...
this_bvm->flags->MC_USE_MALLOC = FLAG_SET; // XXX WAIVER(MC_USE_MALLOC); temporary until large_alloc_list XXX //
    ...
    mword *load_null_bvm = (void*)mem_alloc(this_bvm, UNITS_MTO8(NULL_BVM_SIZE)); // NULL_BVM_SIZE is sfield/4
...
//        _newlfcp(this_bvm, (mword*)opcode_array, NUM_INTERP_OPCODES);
    ...
    this_bvm->interp->jump_table = mem_sys_alloc(UNITS_MTO8(NUM_INTERP_OPCODES));
